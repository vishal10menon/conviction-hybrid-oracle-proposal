...
class GitHubChecker:
    ...
    def get_pr_files(self, owner, repo, pr_number):
        url = f"{self.GITHUB_API}/repos/{owner}/{repo}/pulls/{pr_number}/files"
        response = requests.get(url, headers=self.headers, timeout=30)
        response.raise_for_status()
        return response.json()

    def get_ci_status(self, owner, repo, sha):
        url = f"{self.GITHUB_API}/repos/{owner}/{repo}/commits/{sha}/status"
        response = requests.get(url, headers=self.headers, timeout=30)
        response.raise_for_status()
        return response.json()

    def get_check_runs(self, owner, repo, sha):
        url = f"{self.GITHUB_API}/repos/{owner}/{repo}/commits/{sha}/check-runs"
        response = requests.get(url, headers=self.headers, timeout=30)
        response.raise_for_status()
        return response.json()
...
class PRVerifier:
    ...
    def _check_merge_status(self, pr):
        ...
        return {
            "criterion_id": "pr_merged",
            "type": "github_pr",
            "passed": merged or mergeable,
            "details": f"PR state: {pr['state']}, merged: {merged}, mergeable: {mergeable}",
        }
    ...
    def _check_ci_status(self, ci):
        ...
        all_passed = all(
            run["conclusion"] == "success" for run in ci.get("check_runs", [])
        )
        return {
            "criterion_id": "ci_passing",
            "type": "github_ci",
            "passed": all_passed,
            "details": f"{total} check run(s), all passed: {all_passed}",
        }
    ...
    def _check_files(self, files, manifest):
        ...
        missing = [r for r in required_files if not any(r in c for c in changed_names)]
        return {
            "criterion_id": "files_changed",
            "type": "github_files",
    ...
    def _check_deadline(self, manifest):
        ...
        return {
            ...
            "type": "time",
            "passed": now <= deadline,
            "details": f"Deadline: {deadline_str}, now: {now.isoformat()}",
        }
